...
Describe 'Invoke-LintCommand - commit_paths overlap (in-process, D32)' {
    ...
    # 'check 3'/'check 5' style in tests/Lint.Tests.ps1. Overlap check 15 is skipped
    # under -Lite (_lib.ps1: `if (-not $Lite)`), so these twins never pass -Lite.
    ...
    It 'FAILs two impl tasks sharing a commit_path with no ordering' {
        ...
        ($r.Output -join "`n") | Should -Match "commit_path 'src/foo.txt' also written by 'p-02-b'"
    }
    It 'passes when the two are directly ordered' {
        ...
        $r = Invoke-MusterInProc $script:fx "Invoke-LintCommand -Paths @('tasks/backlog/p-01-a.md','tasks/backlog/p-02-b.md')"
        ($r.Output -join "`n") | Should -Not -Match 'commit_path'
    }
    ...
    It 'FAILs on prefix overlap (dir vs file under it)' {
        ...
        $r = Invoke-MusterInProc $script:fx "Invoke-LintCommand -Paths @('tasks/backlog/p-01-a.md','tasks/backlog/p-02-b.md')"
        $r.ExitCode | Should -Be 1
        ($r.Output -join "`n") | Should -Match 'no depends_on ordering'
    }
    ...
    It 'FAILs two fix-type tasks sharing a commit_path with no ordering' {
        ...
        $r = Invoke-MusterInProc $script:fx "Invoke-LintCommand -Paths @('tasks/backlog/p-01-x.md','tasks/backlog/p-02-y.md')"
        $r.ExitCode | Should -Be 1
        ($r.Output -join "`n") | Should -Match "commit_path 'src/foo.txt' also written by 'p-02-y'"
    }
    It 'emits a finding per unordered overlapping pair (three-way, deterministic)' {
        ...
        $joined | Should -Match "p-01-a.md: commit_path 'src/foo.txt' also written by 'p-02-b'"
        $joined | Should -Match "p-01-a.md: commit_path 'src/foo.txt' also written by 'p-03-c'"
        $joined | Should -Match "p-02-b.md: commit_path 'src/foo.txt' also written by 'p-03-c'"
    }
